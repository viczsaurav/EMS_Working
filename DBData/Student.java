...
@NamedQueries({
    ...
    @NamedQuery(name = "Student.findByStudentId", query = "SELECT s FROM Student s WHERE s.user = (SELECT u FROM User u WHERE u.loginid = :studentId)"),
    @NamedQuery(name = "Student.findByStudentName", query = "SELECT s FROM Student s WHERE s.name = :studentName")})
